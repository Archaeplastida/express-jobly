...
describe("POST /users", function () {
  ...
  test("unauth for users", async function () {
    const resp = await request(app).post("/users").send({ username: "u-new", firstName: "First-new", lastName: "Last-newL", password: "password-new", email: "new@email.com", isAdmin: true, }).set("authorization", `Bearer ${u1Token}`);
...
describe("GET /users/:username", function () {
  ...
  test("works for same user", async function () {
    ...
    expect(resp.body).toEqual({
      user: { username: "u1", firstName: "U1F", lastName: "U1L", email: "user1@user.com", isAdmin: false },
    });
  });
...
describe("PATCH /users/:username", () => {
  ...
  test("works for same user", async function () {
    const resp = await request(app).patch(`/users/u1`).send({ lastName: "Super" }).set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
      user: { username: "u1", firstName: "U1F", lastName: "Super", email: "user1@user.com", isAdmin: false },
    });
  });

  test("unauth for other users", async function () {
    const resp = await request(app).patch(`/users/u1`).send({ lastName: "Super" }).set("authorization", `Bearer ${u2Token}`);
    expect(resp.statusCode).toEqual(401);
  })
...
describe("POST /users/:username/jobs/:id", function () {
  ...
  test("not found for no such username", async function () {
    const resp = await request(app).post(`/users/nope/jobs/${testJobIds[1]}`).set("authorization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(404);
  })
